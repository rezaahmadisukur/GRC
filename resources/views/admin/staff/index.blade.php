...
<x-admin-layout>
    ...
    <div class="py-8">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                ...
                <div
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-emerald-600">{{ $staffs->where('is_active', true)->count() }}
                        </p>
                    </div>
                </div>
                ...
                <div
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-bold text-rose-500">{{ $staffs->where('is_active', false)->count() }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div
                    class="px-6 py-3.5 border-b border-gray-100 flex items-center justify-between bg-gradient-to-r from-slate-50 to-white">
                    ...
                    <table class="w-full">
                        <thead>
                            <tr class="bg-gray-50/70 border-b border-gray-100">
                                <th
                                    class="px-6 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider w-14">
                                    #</th>
                                <th
                                    class="px-6 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">
                                    Nama</th>
                                <th
                                    class="px-6 py-3.5 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">
                                    Username</th>
                                <th
                                    class="px-6 py-3.5 text-center text-xs font-bold text-gray-400 uppercase tracking-wider">
                                    Status</th>
                                <th
                                    class="px-6 py-3.5 text-center text-xs font-bold text-gray-400 uppercase tracking-wider">
                                    Aksi</th>
                            ...
                            @forelse($staffs as $key => $staff)
                                ...
                                : 'bg-emerald-50 text-emerald-700 border border-emerald-200 hover:bg-emerald-100 hover:border-emerald-300'
                                                                ...
                                                                <form id="toggleForm-{{ $staff->id }}"
                                                                    action="{{ route('admin.staff.toggle', $staff->id) }}" method="POST"
                                                                    class="hidden">
                                                                    @csrf
                                                                    @method('PATCH')
                                                                </form>
                                                                ...
                                                                <form id="resetForm-{{ $staff->id }}"
                                                                    action="{{ route('admin.staff.reset-password', $staff->id) }}" method="POST"
                                                                    class="hidden">
                                                                    @csrf
                                                                    @method('PATCH')
                                                                </form>
                                    ...
                                    <td colspan="5">
                                        <div class="py-20 flex flex-col items-center justify-center text-center px-6">
                                            <div class="relative mb-5">
                                                <div
                                                    class="w-24 h-24 rounded-3xl bg-gradient-to-br from-indigo-50 to-violet-100 flex items-center justify-center shadow-inner">
                                                ...
                                                <span
                                                    class="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-indigo-200 animate-bounce"></span>
                                                <span
                                                    class="absolute -bottom-1 -left-1 w-2 h-2 rounded-full bg-violet-200 animate-bounce delay-[200ms]"></span>
                                            </div>
                                            <h3 class="text-base font-bold text-gray-700 mb-1">Belum ada staff terdaftar
                                            </h3>
    ...
    {{-- ════════════════════════════ --}}
    <div id="addModal" class="fixed inset-0 z-50 hidden items-center justify-center p-4" role="dialog" aria-modal="true"
        ...
        <div id="addBox" class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10
            ...
            <form action="{{ route('admin.staff.store') }}" method="POST" id="addStaffForm" novalidate>
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name') }}"
                                placeholder="Masukkan nama lengkap" autocomplete="off" class="w-full pl-10 pr-4 py-2.5 text-sm border rounded-xl outline-none
                    ...
                    {{-- Password Notice --}}
                    <div
                        class="flex items-start gap-2.5 p-3.5 rounded-xl bg-gradient-to-r from-indigo-50 to-violet-50 border border-indigo-100">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-indigo-600 leading-relaxed">
                                ...
                                <code
                                    class="inline-flex items-center gap-1 bg-indigo-100 text-indigo-800 font-mono font-bold px-1.5 py-0.5 rounded-md ml-0.5">grcrental123</code>
                            </p>
                            <p class="text-xs text-indigo-400 mt-1">Bisa diganti kapan saja melalui tombol Reset PW di
                                tabel.</p>
                        </div>
                    </div>
                ...
                <div class="px-6 py-4 border-t border-gray-100 flex justify-end gap-2.5 bg-gray-50/50">
                    <button type="button" onclick="closeModal()" class="px-5 py-2.5 text-sm font-semibold text-gray-600 rounded-xl bg-white border border-gray-200
    ...
    {{-- ════════════════════════════════════ --}}
    <div id="toggleModal" class="fixed inset-0 z-[60] hidden items-center justify-center p-4" role="dialog"
        aria-modal="true">
        ...
        <div id="toggleBox" class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm z-10
                transform transition-all duration-300 scale-90 opacity-0 overflow-hidden">
            ...
            <div class="p-6">
                ...
                <div id="toggleInfoBox" class="flex items-start gap-2.5 p-3 rounded-xl border mb-6">
                    <svg id="toggleInfoIcon" class="w-4 h-4 shrink-0 mt-0.5" fill="none" stroke="currentColor"
                        viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex gap-3">
                    <button type="button" onclick="closeToggleModal()" class="flex-1 py-2.5 text-sm font-semibold text-gray-600 rounded-xl
                           bg-gray-100 hover:bg-gray-200 border border-gray-200
                           transition-all duration-200 active:scale-95">
                        Batal
                    </button>
                    <button type="button" id="toggleConfirmBtn" class="flex-1 py-2.5 text-sm font-semibold text-white rounded-xl
    ...
    {{-- ════════════════════════════════════ --}}
    <div id="resetModal" class="fixed inset-0 z-[60] hidden items-center justify-center p-4" role="dialog"
        aria-modal="true">
        ...
        <div id="resetBox" class="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10
                transform transition-all duration-300 scale-90 opacity-0 overflow-hidden">

            <div
                class="bg-gradient-to-br from-violet-500 via-indigo-500 to-blue-500 px-6 pt-6 pb-10 relative overflow-hidden">
                ...
                <button type="button" onclick="closeResetModal()" class="absolute top-4 right-4 p-1.5 rounded-lg text-white/70 hover:text-white
            ...
            <div class="mt-6 mx-4 bg-white rounded-2xl shadow-lg border border-gray-100 p-5 mb-4">
                ...
                <div>
                    ...
                    <div
                        class="flex items-center justify-between gap-3 px-4 py-3 rounded-xl bg-gray-50 border border-gray-200">
                        ...
                        <span
                            class="text-xs font-semibold text-indigo-600 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded-full">default</span>
                    </div>
                </div>
            ...
            <div class="px-4 pb-5 flex gap-3">
                <button type="button" onclick="closeResetModal()" class="flex-1 py-2.5 text-sm font-semibold text-gray-600 rounded-xl
                       bg-gray-100 hover:bg-gray-200 border border-gray-200
                       transition-all duration-200 active:scale-95">
                    Batal
                </button>
                <button type="button" id="resetConfirmBtn" class="flex-1 py-2.5 text-sm font-semibold text-white rounded-xl
