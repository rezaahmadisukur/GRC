...
<aside
  ...
  <nav class="px-4 space-y-1.5 h-[calc(100vh-180px)] overflow-y-auto custom-scrollbar">
    <div class="sidebar-label mb-4 mt-6" x-show="!sidebarCollapsed" x-cloak>
  ...
  : 'text-slate-500 hover:bg-slate-50 hover:text-slate-900' }}" :class="{ 'justify-center': sidebarCollapsed }">
    ...
    @if (Auth::user()->role === 'owner')
      <div class="sidebar-label pt-4 mb-2" x-show="!sidebarCollapsed" x-cloak>
        <p class="px-4 text-[11px] font-bold text-slate-400 uppercase tracking-widest">
          Management
        </p>
      </div>
  ...
  <div class="absolute bottom-0 w-full p-4 bg-white/80 backdrop-blur-md border-t border-slate-50">
    <div class="bg-slate-50 rounded-2xl p-3">
      ...
      <!-- User Info -->
      <div class="sidebar-user-wrapper flex items-center gap-3 px-4"
        :class="{ 'justify-center px-0': sidebarCollapsed }">
        <div class="w-10 h-10 rounded-xl bg-white border border-slate-200 flex items-center 
                      justify-center text-emerald-600 shadow-sm flex-shrink-0">
          <span class="font-bold text-lg">{{ substr(Auth::user()->name, 0, 1) }}</span>
        </div>

        <!-- ✅ Tambah class sidebar-user-info -->
        <div class="sidebar-user-info overflow-hidden" x-show="!sidebarCollapsed" x-cloak>
          <p class="text-sm font-bold text-slate-800 truncate">{{ Auth::user()->name }}</p>
          <p class="text-[11px] text-slate-500 truncate font-medium">{{ Auth::user()->email }}</p>
        </div>
      </div>

      <!-- ✅ Tambah class sidebar-actions -->
      <div class="sidebar-actions mt-3 flex gap-2 items-center"
        :class="{ 'flex-col gap-1.5 mx-auto items-center': sidebarCollapsed }">

        <!-- Profile -->
        <a href="{{ route('profile.edit') }}" class="flex-1 flex justify-center items-center gap-3 py-3 rounded-xl bg-white border 
                   border-slate-200 text-slate-600 hover:bg-slate-100 transition-all duration-300 group"
          ...
          <svg class="w-5 h-5 flex-shrink-0 group-hover:scale-105 transition-transform duration-300" fill="none"
            ...
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 
        ...
        <form method="POST" action="{{ route('logout') }}" class="flex-1">
          @csrf
          <button type="submit" class="w-full flex justify-center items-center gap-3 py-3 rounded-xl bg-red-50 border 
                     border-red-100 text-red-500 hover:bg-red-100 transition-all duration-300"
            :class="{ 'px-2': sidebarCollapsed }" title="Logout">
            <svg class="w-5 h-5 flex-shrink-0 group-hover:scale-105 transition-transform duration-300" fill="none"
              stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 
                     3 0 013 3v1" />
            </svg>
            <!-- ✅ Tambah class sidebar-label -->
            <span class="sidebar-label text-[14px] font-semibold" x-show="!sidebarCollapsed" x-cloak>
